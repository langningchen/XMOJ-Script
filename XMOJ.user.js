    ...
    GM_xmlhttpRequest({
        ...
        onload: (Response) => {
            if (Response.status === 508) {
                CallBack({
                    "Success": false, "ErrorMessage": "服务器连接数过多，请稍后再试"
                });
                return
            }
            if (Response.status !== 200) {
                CallBack({
                    "Success": false, "ErrorMessage": "服务器返回错误码：" + Response.status
                });
                return;
            }
            let ResponseData;
            try {
                ...
                if (Response.responseText.indexOf("parking") != -1) {
                    CallBack({
                        "Success": false, "ErrorMessage": "在访问XMOJ的时候请不要打开VPN"
                    });
                    return;
                }
                ResponseData = {
                    "Success": false, "ErrorMessage": "JSON解析错误：" + error
                };
            }
    ...
    if (document.querySelector("#navbar") != null) {
        ...
        if (location.pathname == "/index.php" || location.pathname == "/") {
            ...
            if (document.querySelector("#rank") == null) {
                document.querySelector("body > div > div.mt-3").innerHTML = "<div style="text-align: center;"><h3>比赛排名</h3><a></a><table id=\"rank\"></table>";
            }
            if (new URL(location.href).searchParams.get("ByUserScript") == null) {
            ...
            document.querySelector("body > div > div.mt-3").innerHTML = `<div class="mb-3" style="text-align: center;">` + `<h3>提交代码</h3>` + (SearchParams.get("id") != null ? `题目<span class="blue">${SearchParams.get("id")}</span>` : `比赛<span class="blue">${SearchParams.get("cid") + `</span>&emsp;题目<span class="blue">` + String.fromCharCode(65 + parseInt(SearchParams.get("pid")))}</span>`) + `</div>
            ...
            if (SearchParams.get("other") == null) {
                ...
                AddUser.addEventListener("click", () => {
                    ...
                    RequestAPI("Mail", "SendMail", {
                        "ToUser": UsernameData, "Content": "你好，我是" + localStorage.getItem("UserScript-Username")
                    }, (ResponseData) => {
                ...
                Send.addEventListener("click", () => {
                    ...
                    RequestAPI("Mail", "SendMail", {
                        "ToUser": SearchParams.get("other"), "Content": ContentData
                    }, (ResponseData) => {
                        Send.disabled = false;
                    ...
                    if (ResponseData.Success == true) {
                        ...
                        for (let i = 0; i < Posts.length; i++) {
                            PostList.children[1].innerHTML += `<tr>
                                    ...
                                    <td><a href="/userinfo.php?user=${Posts[i].UserID}">${Posts[i].UserID}</a></td>` + (Posts[i].ProblemID == null ? `<td></td>` : `<td><a href="/problem.php?id=${Posts[i].ProblemID}">${Posts[i].ProblemID}</a></td>`) + `<td>${Posts[i].PostTime}</td>
            ...
            <div id="ErrorElement" class="alert alert-danger" role="alert" style="display: none;"></div>`;
                ...
                SubmitElement.addEventListener("click", async () => {
                    ...
                    RequestAPI("BBS", "NewPost", {
                        "Title": Title, "Content": Content, "ProblemID": ProblemID
                    }, (ResponseData) => {
                ...
                if (SearchParams.get("tid") == null) {
                    ...
                    SubmitElement.addEventListener("click", async () => {
                        ...
                        RequestAPI("BBS", "NewReply", {
                            "PostID": SearchParams.get("tid"), "Content": ContentElement.value
                        }, async (ResponseData) => {
                            SubmitElement.disabled = false;
                            SubmitElement.children[0].style.display = "none";
                            if (ResponseData.Success == true) {
